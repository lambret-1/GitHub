...
| 参数/属性 | 中文释义 | 说明 |
|-----------|---------|------|
| `owner` | 仓库所有者 | 仓库所有者用户名 |
| `repo` | 仓库名称 | 仓库名称 |
| `branch` | 分支名称 | 当前分支名称 |
| `onJumpToCode` | 跳转回调 | 点击代码片段后的跳转闭包回调（替代NotificationCenter） |
| `searchQuery` | 搜索关键词 | 搜索输入框的文本内容 |
| `searchResults` | 搜索结果 | 代码搜索结果数组（CodeSearchItem） |
| `isSearching` | 搜索中 | 搜索请求进行中的状态标记 |
| `errorMessage` | 错误信息 | 搜索失败时的错误提示文本 |
| `selectedItem` | 选中项 | 当前选中的搜索结果项 |
| `showCodeSnippet` | 显示代码片段 | 控制代码片段弹窗的显示 |
| `searchTask` | 防抖任务 | 防抖搜索Task，300ms延迟，视图消失时自动取消 |
| `currentSearchWorkItem` | 搜索任务 | 当前搜索请求DispatchWorkItem，用于取消旧请求解决竞态 |

**核心功能（生产级）：**
- 防抖搜索：输入300ms后自动搜索，Task可取消
- 竞态防护：新搜索取消旧请求，回调校验查询词，防止旧结果覆盖新结果
- 分支搜索：传入branch参数（GitHub API预留扩展）
- 搜索结果统计：修正文案为"共找到N个文件包含匹配"
- 代码片段查看：点击搜索结果打开代码片段弹窗
- 关键词高亮：使用AttributedString实现，原始字符串range搜索，避免Unicode崩溃
- 行号显示：每个代码片段显示匹配的行号
- 点击跳转：使用闭包回调跳转到代码编辑页面对应行
- 任务生命周期：视图消失时自动取消所有后台任务，防止内存泄漏

**页面元素：**
- 搜索输入框（关闭自动大写/拼写纠错，支持实时防抖搜索）
- 清除按钮（输入框非空时显示）
- 搜索结果统计栏（修正文案语义）
- 搜索结果列表（文件名、路径中间截断、匹配提示）
- 代码片段弹窗（高亮显示匹配代码）

**CodeSnippetView（代码片段弹窗）：**
- NavigationStack包裹（iOS16.5+推荐API）
- 文件信息头部（文件名、路径中间截断）
- 代码片段列表（行号、点击跳转提示、代码内容）
- 关键词高亮（AttributedString，黄色背景+红色文字+粗体）
- 手势隔离：仅头部信息栏可点击跳转，代码区域独立ScrollView横向滚动
- 后台解析：大文件片段提取在后台线程执行，避免UI卡顿
- 网络取消：视图消失时取消文件加载请求

**CodeSnippet（代码片段模型）：**
- `lineNumber`：匹配行号
- `code`：代码片段内容（包含前后2行上下文，连续命中行合并为一个片段）

**安全与性能优化：**
- 移除NotificationCenter，改用闭包回调，避免订阅者泄漏
- 移除[AnyView]类型擦除，改用AttributedString，提升渲染性能
