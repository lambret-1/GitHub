...
- 下载功能（自包含，不依赖父视图）：
  - 点击下载按钮后先获取该标签的Release信息（GET /repos/{owner}/{repo}/releases/tags/{tag}）
  - 弹出独立全屏资产选择页，展示：
    - Release资产列表（文件名+大小+下载次数），如果该标签有Release
    - 源代码下载选项（ZIP格式、TAR.GZ格式），始终显示
  - 用户选择某个资产/源代码格式后，弹出下载确认弹窗询问是否下载
  - 确认后显示半透明进度浮层（标签名 + 进度条 + 百分比）
...
23. **仓库设置**：查看仓库设置和统计信息
24. **下拉刷新**：在仓库页面下拉即可刷新，并行刷新文件列表（含README、最新提交）、分支列表、标签列表、星标状态，使用SwiftUI原生.refreshable实现，流畅动画反馈
25. **左滑手势**：支持左滑返回上级目录
