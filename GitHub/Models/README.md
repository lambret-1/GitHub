...
| 属性名 | 中文释义 | 说明 |
|--------|---------|------|
| `name` | 标签名称 | Git 标签名（如 "v1.2.3"） |
| `commit` | 指向的提交 | 标签指向的提交信息（TagCommit 子模型） |
| `id` | 唯一标识 | 计算属性，使用标签名作为 Identifiable 的 id |
| `短码` | 提交短码 | 计算属性，返回提交 SHA 前 7 位 |

**TagCommit 子模型：**

| 属性名 | 中文释义 | 说明 |
|--------|---------|------|
| `sha` | 提交哈希 | 标签指向的 Git 提交 SHA |
